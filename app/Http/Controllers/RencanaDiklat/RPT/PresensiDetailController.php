...
{
    // 1. Ambil data periode dan jam_diklat
    ...
    // 2. PERBAIKAN: Ambil jawaban HANYA untuk detail_id DAN periode_id ini
    $answers = UserAnswerPostPreeDetail::with('question.test')
        ->whereHas('question.test', function ($query) use ($detail_id, $periode_id) {
            $query->where('detail_program_id', $detail_id)
                  ->where('periode_id', $periode_id); // <-- TAMBAHKAN BARIS INI
        })->get();

    // 3. Ekstrak NRP unik yang sudah mengisi
    ...
    // 4. Ambil data karyawan yang NRP-nya ada di jawaban
    $karyawanMap = Karyawans::whereIn('nrp', $nrpsWithAnswers)
        ->pluck('nama_karyawan', 'nrp'); 

    // 5. Hitung nilai per NRP
    $scoreMap = [];
    ...
    // 6. Gabungkan jadi array untuk kirim ke Vue
    $karyawanDenganNilai = [];
