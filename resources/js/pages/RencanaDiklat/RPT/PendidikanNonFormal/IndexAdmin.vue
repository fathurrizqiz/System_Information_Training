...
import DetailEksternalModal from '@/pages/RencanaDiklat/RPT/PendidikanNonFormal/DetailEksternalModalAdmin.vue';
import ProgramEksternalModal from '@/pages/RencanaDiklat/RPT/PendidikanNonFormal/ProgramEksternalModalAdmin.vue';
...
// Get data from controller (TANPA templates)
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="group relative h-10 w-full overflow-hidden rounded-s-lg shadow-lg/30 md:h-32">
            <!-- 1. Background Image -->
            <img
            ...
            <div class="absolute inset-0 flex flex-col justify-end p-6 md:p-10">
                <h1 class="text-xl font-bold tracking-tight text-white drop-shadow-lg md:text-3xl">
                    Pelatihan Eksternal (ADMIN)
                </h1>
                <p class="mt-2 translate-y-4 transform text-sm text-gray-200 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100 md:text-base">
        ...
        <div class="space-y-6 p-4 md:p-6 lg:p-8">
            ...
            <div class="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:flex-row md:items-end dark:border-slate-800 dark:bg-slate-900">
                <div class="flex-1">
                    <!-- Dropdown templates telah dihapus dari sini -->
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold tracking-wider text-slate-500 uppercase">Cari Program</label>
                        <div class="relative">
                            <input
                                v-model="searchQuery"
                                @input="resetToPage1"
                            ...
                            />
                            <svg class="absolute top-2.5 left-3 h-5 w-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="w-full md:w-48">
                    <label class="mb-1.5 block text-xs font-semibold tracking-wider text-slate-500 uppercase">Tahun</label>
                    <select
                ...
                <div class="w-full md:w-48">
                    <label class="mb-1.5 block text-xs font-semibold tracking-wider text-slate-500 uppercase">Tampilan</label>
                    <select
            ...
            <div class="text-sm font-medium text-slate-500 dark:text-slate-400">
                Menampilkan
                <span class="text-slate-900 dark:text-white">{{ paginatedPrograms.length }}</span>
                dari
                <span class="text-slate-900 dark:text-white">{{ filteredPrograms.length }}</span>
                program
            </div>
                ...
                >
                    ...
                    <div class="p-5">
                        <div v-if="prog.eksternal && prog.eksternal.length > 0" class="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700">
                            <table class="w-full text-left text-sm whitespace-nowrap">
                                <thead class="bg-slate-50 text-slate-500 dark:bg-slate-800/50 dark:text-slate-400">
                                    <tr>
                                        <th class="px-4 py-3 font-semibold tracking-wider uppercase">Nama Karyawan</th>
                                        <th class="px-4 py-3 font-semibold tracking-wider uppercase">NRP</th>
                                        <th class="px-4 py-3 font-semibold tracking-wider uppercase">Diklat</th>
                                        <th class="px-4 py-3 font-semibold tracking-wider uppercase">Tanggal Mulai</th>
                                        <th class="px-4 py-3 font-semibold tracking-wider uppercase">Tanggal Selesai</th>
                                        <th class="px-4 py-3 font-semibold tracking-wider uppercase">Jam Diklat</th>
                                        <th class="px-4 py-3 font-semibold tracking-wider uppercase">Status</th>
                                        <th class="px-4 py-3 font-semibold tracking-wider uppercase">Alasan</th>
                                        <th class="px-4 py-3 text-center font-semibold tracking-wider uppercase">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                    <tr v-for="detail in prog.eksternal" :key="detail.id" class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/25">
                                        ...
                                        <td class="px-4 py-3 text-slate-600 dark:text-slate-300">
                                            <button
                                                ...
                                                @click="openReasonModal(detail.catatan_penolakan)"
                                        ...
                                        <td class="flex justify-center gap-1 px-4 py-3 text-center">
                                            <button
                                            ...
                                            <!-- Tombol Notifikasi telah dihapus dari sini -->
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <div v-else class="flex flex-col items-center justify-center py-8 text-slate-500 dark:text-slate-400">
                            <svg class="mb-2 h-10 w-10 text-slate-300 dark:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        >
            <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-900">
                ...
                <div class="max-h-60 overflow-y-auto rounded-lg bg-slate-50 p-4 text-sm text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                    {{ selectedReason }}
                </div>
