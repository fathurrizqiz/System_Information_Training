...
const handleFileUpload = (event: Event) => {
    ...
    if (target.files && target.files.length > 0) {
        form.value.bukti_hadir = target.files[0];
    }
};
...
const submit = () => {
    ...
    if (!form.value.bukti_hadir && !props.detail) {
        toast.error('Sertifikat harus diunggah');
        return;
    }

    const url = props.detail
        ? `/Admin/Eksternal/storeDetail/update/${props.detail.id}`
        : '/Admin/Eksternal/storeDetail';

    // 1. Buat salinan data agar state asli form tidak rusak
    const dataToSend = { ...form.value };

    // 2. JIKA UPDATE: Tambahkan _method: 'put' untuk menipu route Laravel
    if (props.detail) {
        dataToSend._method = 'put';
    }

    // 3. Kirim dataToSend, BUKAN form.value
    router.post(url, dataToSend, {
        forceFormData: true, // Pastikan Inertia membungkusnya sebagai FormData agar file terbaca
        onSuccess: () => {
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="col-span-2">
                    <label class="mb-1 block text-sm font-medium"
                        >Upload Sertifikat (PDF / JPG)</label
                    >
                    <input
                        type="file"
                        accept=".pdf,.jpg,.jpeg"
